-- =====================================================
-- CREATE SYSTEM_PROMPTS TABLE AND INSERT AI PROMPT
-- =====================================================
...
-- Step 3: Insert or update the dental matcher AI prompt
INSERT INTO system_prompts (prompt_name, prompt_text, is_active, created_at, updated_at)
VALUES (
  'dental_matcher_default',
  '# Dental Recruitment AI Assistant
...
-- Step 4: Create the RPC function to get active system prompts (if it doesn't exist)
CREATE OR REPLACE FUNCTION get_active_system_prompt(p_prompt_name TEXT)
RETURNS TEXT
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
GRANT EXECUTE ON FUNCTION get_active_system_prompt(TEXT) TO authenticated;

-- Success message
DO $$
BEGIN
  RAISE NOTICE '✅ System prompts table created successfully!';
  RAISE NOTICE '✅ AI prompt inserted/updated!';
  RAISE NOTICE '✅ RPC function created!';
  RAISE NOTICE 'Your AI assistant now has full capabilities!';
END $$;
