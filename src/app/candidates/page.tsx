...
export default function CandidatesPage() {
  ...
  const loadColumnSettings = async () => {
    try {
      const result = await loadColumnPreferences('candidates');
      ...
      if (result.success) {
        ...
        if (result.columnWidths) {
          ...
          saveColumnPreferences('candidates', defaultColumnWidths, false);
        }
        ...
        if (result.isLocked !== null && result.isLocked !== undefined) {
          ...
          saveColumnPreferences('candidates', columnWidths, false);
        }
      } else {
        ...
        const saveResult = await saveColumnPreferences('candidates', defaultColumnWidths, false);
        ...
        try {
          const savedWidths = localStorage.getItem('candidates-table-column-widths');
          const savedLockState = localStorage.getItem('candidates-table-column-locked');
  ...
  const saveColumnSettings = async (widths: Record<string, number>, locked: boolean) => {
    try {
      const result = await saveColumnPreferences('candidates', widths, locked);
      
      if (!result.success) {
        console.warn('Failed to save column settings to database:', result.error);
        // Fallback to localStorage
        try {
          localStorage.setItem('candidates-table-column-widths', JSON.stringify(widths));
          localStorage.setItem('candidates-table-column-locked', JSON.stringify(locked));
        } catch (localError) {
            ...
            <table ref={tableRef} className="w-full table-fixed divide-y divide-gray-300">
              <thead className="bg-gray-100">
                <tr>
                  ...
                  <th
                    className={`px-2 py-3 text-left text-xs font-bold text-gray-900 uppercase border-r border-gray-300 relative group ${selectedSalaries.length > 0 ? 'bg-blue-100' : ''}`}
                  ...
                  <th
                    className={`px-2 py-3 text-left text-xs font-bold text-gray-900 uppercase border-r border-gray-300 relative group ${selectedAvailabilities.length > 0 ? 'bg-blue-100' : ''}`}
                  ...
                  <th
                    className={`px-2 py-3 text-left text-xs font-bold text-gray-900 uppercase border-r border-gray-300 relative group ${selectedExperiences.length > 0 ? 'bg-blue-100' : ''}`}
                  ...
                  <tr key={candidate.id} className="hover:bg-gray-50">
                    ...
                    >
                      <div className="flex items-center">
                        <HoverableCell 
                          ...
                          label="Candidate ID"
                          onCopy={() => {
                            console.log(`Copied candidate ID: ${candidate.id}`);
                          }}
                        />
                    ...
                    >
                      {isEditMode ? (
                        ...
                        <HoverableCell value={candidate.last_name} label="Last Name" />
                      )}
                    </td>
                    <td
                      className="px-3 py-2 whitespace-nowrap text-sm font-medium text-gray-900 border-r border-gray-300 overflow-hidden text-ellipsis"
                    ...
                    >
                      {isEditMode ? (
                        ...
                        <HoverableCell value={candidate.phone} label="Phone" />
                      )}
                    </td>
                    <td
                      className="px-3 py-2 whitespace-nowrap text-sm font-medium text-gray-900 border-r border-gray-300 overflow-hidden text-ellipsis"
                    ...
                    >
                      {isEditMode ? (
                        ...
                        <HoverableCell value={normalizeRole(candidate.role)} label="Role" />
                      )}
                    </td>
                    <td
                    ...
                    >
                      {isEditMode ? (
                        ...
                        <HoverableCell value={candidate.postcode} label="Postcode" />
                      )}
                    </td>
                    <td
                      className="px-3 py-2 whitespace-nowrap text-sm font-medium text-gray-900 border-r border-gray-300 overflow-hidden text-ellipsis"
                    ...
                    >
                      {isEditMode ? (
                        ...
                        <HoverableCell value={candidate.salary} label="Salary" />
                      )}
                    </td>
                    <td
                      className="px-3 py-2 whitespace-nowrap text-sm font-medium text-gray-900 border-r border-gray-300 overflow-hidden text-ellipsis"
                    ...
                    >
                      {isEditMode ? (
                        ...
                        <HoverableCell value={candidate.days} label="Days" />
                      )}
                    </td>
                    <td
                      className="px-3 py-2 whitespace-nowrap text-sm font-medium text-gray-900 border-r border-gray-300 overflow-hidden text-ellipsis"
                    ...
                    >
                      {isEditMode ? (
                        ...
                        <HoverableCell value={candidate.experience} label="Experience" />
                      )}
                    </td>
                    <td
