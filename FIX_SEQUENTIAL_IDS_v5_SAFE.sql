-- ============================================
-- FIX: Auto-generate sequential IDs per user (v5 - SAFE VERSION)
-- ============================================
-- Run this in Supabase SQL Editor
-- ============================================
-- This creates IDs like: CAN1, CAN2, CAN3... and CL1, CL2, CL3...
-- Each user has their own sequence starting from 1
-- Handles existing data safely without conflicts
-- ============================================

-- STEP 1: DROP ALL POLICIES ON CANDIDATES AND CLIENTS
-- ============================================
...
-- STEP 2: DROP POLICIES ON RELATED TABLES
-- ============================================
...
-- STEP 5: CLEAR EXISTING DATA - RENAME TO TEMP IDs
-- ============================================
-- This prevents conflicts by ensuring no CAN1, CAN2, etc. exist yet
...
UPDATE matches
SET candidate_id = 'TEMP_' || candidate_id
WHERE candidate_id NOT LIKE 'TEMP_%';

UPDATE matches
SET client_id = 'TEMP_' || client_id
WHERE client_id NOT LIKE 'TEMP_%';

-- STEP 6: NOW UPDATE TO SEQUENTIAL IDS (NO CONFLICTS!)
-- ============================================

-- Update candidates to sequential IDs
UPDATE candidates
...
-- Update clients to sequential IDs
UPDATE clients
SET id = subquery.new_id
FROM (
  SELECT
...
-- Update matches table to use new sequential IDs
UPDATE matches m
SET candidate_id = (
  SELECT c.id
  FROM candidates c
  WHERE 'TEMP_' || c.id = m.candidate_id
    OR c.id = m.candidate_id
  LIMIT 1
);
...
SET client_id = (
  SELECT cl.id
  FROM clients cl
  WHERE 'TEMP_' || cl.id = m.client_id
    OR cl.id = m.client_id
  LIMIT 1
);

-- STEP 7: CREATE AUTO-INCREMENT FUNCTIONS
-- ============================================
...
-- Safe to run multiple times - handles existing CAN/CL IDs
